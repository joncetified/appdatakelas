...
@section('content')
        <section class="panel px-5 py-6 sm:px-7">
            <p class="text-xs font-semibold uppercase tracking-[0.34em] text-slate-500">Masuk Ke Sistem</p>
            <h2 class="mt-3 text-2xl font-semibold text-slate-950">Login</h2>
            <p class="mt-2 text-sm leading-6 text-slate-600">
                {{ $needsInitialSetup
                    ? 'Akun pertama harus dibuat lewat setup awal.'
                    : 'Masukkan nama lengkap atau email dan password untuk masuk.' }}
            </p>
            ...
            <a
                ...
                class="mt-6 flex w-full items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
            >
            ...
            <form method="POST" action="{{ route('login.store') }}" class="mt-6 space-y-5">
                ...
                <div class="flex flex-wrap justify-between gap-3 text-sm text-slate-600">
                    ...
                    @else
                    <span>Akun Google baru otomatis masuk sebagai ketua kelas.</span>
                    @endif
