...
@section('content')
    <section class="panel px-6 py-6 lg:px-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="safe-wrap">
                ...
                <h2 class="safe-wrap mt-3 text-3xl font-semibold text-slate-950">{{ $report->classroom?->name ?? 'Kelas tidak tersedia' }}</h2>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <article class="{{ $report->critical_stock_count > 0 ? 'panel critical-stock-card' : 'panel' }} px-5 py-5">
            <p class="text-sm text-slate-500">Kondisi Kritis</p>
        ...
        <article class="panel px-6 py-6 lg:px-8">
            ...
            <div class="mt-5 grid gap-4 text-sm text-slate-600 md:grid-cols-2">
                <div>
                    ...
                    <p class="safe-wrap mt-1">{{ $report->reporter?->name ?? '-' }}</p>
                </div>
                <div>
                    <p class="font-semibold text-slate-950">Ketua Kelas</p>
                    <p class="safe-wrap mt-1">{{ $report->classroom?->leader?->name ?? '-' }}</p>
                </div>
                <div>
                    <p class="font-semibold text-slate-950">Wali Kelas</p>
                    <p class="safe-wrap mt-1">{{ $report->classroom?->homeroomTeacher?->name ?? '-' }}</p>
                </div>
                <div>
                    <p class="font-semibold text-slate-950">Diverifikasi Oleh</p>
                    <p class="safe-wrap mt-1">{{ $report->verifier?->name ?? '-' }}</p>
                </div>
                <div>
            ...
            <div class="mt-6">
                ...
                <p class="safe-wrap mt-2 text-sm leading-6 text-slate-600">{{ $report->notes ?: 'Tidak ada catatan umum.' }}</p>
            </div>

            <div class="mt-6">
                <p class="font-semibold text-slate-950">Catatan Verifikasi</p>
                <p class="safe-wrap mt-2 text-sm leading-6 text-slate-600">{{ $report->verification_notes ?: 'Belum ada catatan verifikasi.' }}</p>
            </div>

            @if (auth()->user()->isSuperAdmin())
                <div class="mt-6 rounded-3xl bg-slate-50 px-5 py-5 text-sm text-slate-600">
                    <p class="safe-wrap">Dibuat oleh: <span class="font-semibold text-slate-950">{{ $report->createdByUser?->name ?? $report->reporter?->name ?? '-' }}</span></p>
                    <p class="safe-wrap mt-1">Diubah oleh: <span class="font-semibold text-slate-950">{{ $report->updatedByUser?->name ?? '-' }}</span></p>
                </div>
            @endif
        </article>
            ...
            <article class="panel px-6 py-6 lg:px-8">
                ...
                <form method="POST" action="{{ route('reports.verification.update', $report) }}" class="mt-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="verification_notes" name="verification_notes" rows="5" maxlength="1000" class="field mt-2">{{ old('verification_notes', $report->verification_notes) }}</textarea>
                    </div>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @foreach ($report->items as $item)
                        <tr class="{{ $item->is_critical_stock ? 'critical-stock-row' : '' }} align-top">
                            <td class="safe-wrap px-6 py-4 font-medium text-slate-950">{{ $item->item_name }}</td>
                            <td class="cell-nowrap px-6 py-4 text-slate-600">{{ $item->total_units }}</td>
                            <td class="cell-nowrap px-6 py-4 text-slate-600">{{ $item->good_units }}</td>
                            <td class="cell-nowrap px-6 py-4 text-slate-600">{{ $item->damaged_units }}</td>
                            <td class="safe-wrap px-6 py-4 text-slate-600">
                                ...
                                <p class="safe-wrap mt-2">{{ $item->notes ?: '-' }}</p>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
