@extends('layouts.app')

@section('content')
    <section class="panel px-6 py-6 lg:px-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.34em] text-slate-500">Admin</p>
                <h2 class="mt-3 text-3xl font-semibold text-slate-950">Kelola Kelas</h2>
                <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                    Atur nama kelas, lokasi, ketua kelas, dan wali kelas. Penautan ini menentukan hak input dan verifikasi.
                </p>
            </div>
            <a href="{{ route('admin.classrooms.create') }}" class="btn-primary">Tambah Kelas</a>
        </div>
    </section>

    <section class="panel px-6 py-5 lg:px-8">
        <form method="GET" class="flex flex-col gap-4 md:flex-row md:items-end">
            <div class="w-full md:max-w-md">
                <label for="q" class="label">Cari Kelas</label>
                <input
                    id="q"
                    name="q"
                    type="text"
                    value="{{ request('q') }}"
                    class="field mt-2"
                    placeholder="Nama kelas, lokasi, ketua kelas, atau wali kelas"
                >
            </div>
            <button type="submit" class="btn-secondary">Cari</button>
        </form>
    </section>

    <section class="panel overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50 text-left text-slate-500">
                    <tr>
                        <th class="px-6 py-4 font-semibold">Kelas</th>
                        <th class="px-6 py-4 font-semibold">Lokasi</th>
                        <th class="px-6 py-4 font-semibold">Ketua Kelas</th>
                        <th class="px-6 py-4 font-semibold">Wali Kelas</th>
                        <th class="px-6 py-4 font-semibold">Laporan</th>
                        @if (auth()->user()->isSuperAdmin())
                            <th class="px-6 py-4 font-semibold">Audit</th>
                        @endif
                        <th class="px-6 py-4 font-semibold">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($classrooms as $classroom)
                        <tr class="align-top">
                            <td class="px-6 py-4">
                                <p class="font-medium text-slate-950">{{ $classroom->name }}</p>
                                <p class="mt-1 text-slate-500">{{ $classroom->description ?: 'Tanpa deskripsi' }}</p>
                            </td>
                            <td class="px-6 py-4 text-slate-600">{{ $classroom->location ?: '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $classroom->leader?->name ?? 'Belum ditentukan' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $classroom->homeroomTeacher?->name ?? 'Belum ditentukan' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $classroom->reports_count }}</td>
                            @if (auth()->user()->isSuperAdmin())
                                <td class="px-6 py-4 text-slate-600">
                                    <p>Dibuat: {{ $classroom->createdByUser?->name ?? '-' }}</p>
                                    <p class="mt-1">Diubah: {{ $classroom->updatedByUser?->name ?? '-' }}</p>
                                </td>
                            @endif
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-3">
                                    <a href="{{ route('admin.classrooms.edit', $classroom) }}" class="btn-secondary">Edit</a>
                                    <form method="POST" action="{{ route('admin.classrooms.destroy', $classroom) }}" onsubmit="return confirm('Hapus kelas ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-sm font-semibold text-rose-600 underline underline-offset-4">Hapus</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()->isSuperAdmin() ? '7' : '6' }}" class="px-6 py-10 text-center text-slate-500">Belum ada data kelas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        <div class="border-t border-slate-200 px-6 py-4">
            {{ $classrooms->links() }}
        </div>
    </section>
@endsection
